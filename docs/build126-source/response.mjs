...
export function sampleRideResponse(sim,out={},menu=false){
 out.compression=0;out.kind='steady';out.impact=0;out.enabled=false;out.slide=0;out.grip=0;out.yank=0;out.load=0;out.brake=0;out.flight=0;out.rampAir=null;out.release=null;out.coast=0;out.speedPosture=0;
 ...
 // Steady posture yields to every stronger response; it never changes physics.
 if(!r.trick&&r.height<=.01&&r.up<=0){
  out.release=r.motionRelease??(r.holding?0:1);
  const quiet=clamp(1-Math.abs(out.slide)*1.5-out.yank*1.5-out.compression*2-out.brake);
  out.speedPosture=.20*clamp(r.motionSpeed??0)*quiet;
  out.coast=!r.holding?.18*out.release*quiet:0;
  if(out.kind==='steady'&&!r.holding)out.kind=out.release<.999?'ROPE_RELEASE':'FREE_COAST';
  out.compression=Math.max(out.compression,out.speedPosture);
 }
