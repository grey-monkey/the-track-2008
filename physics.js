...
export class Ride {
 ...
 ollie(){const r=this.rider;if(this.phase!=='riding'||!RIDERS[this.options.rider].ollie||!BOARDS[this.options.board].ollie||r.height>0)return false;r.up=2.4;return true;}
 crash(reason){if(this.phase==='crashed'||this.phase==='ended')return;this.phase='crashed';this.phaseTime=0;this.rider.holding=false;this.rider.tension=0;this.stats.crashReason=reason;this.board={x:this.rider.x,z:this.rider.z,vx:this.rider.vx*1.06,vz:this.rider.vz*1.06,heading:this.rider.heading};}
 step(input={},dt=STEP){if(this.phase==='ended'||this.phase==='regroup')return;this.time+=dt;this.phaseTime+=dt;const c=this.car,r=this.rider,oldHx=c.hx,oldHz=c.hz,oldX=r.x,oldZ=r.z;
 // On a release the driver stays ahead of the coasting rider while slowing.
 // Slamming on the brakes here would cause an unavoidable rear-end collision.
 const coastSpeed=Math.hypot(r.vx,r.vz);
 const target=this.phase==='riding'?this.targetSpeed():this.phase==='coasting'&&coastSpeed>.3?coastSpeed+.3:0;
 ...
 let sp=Math.hypot(r.vx,r.vz);const onRoad=roadDistance(r.x,r.z);r.dirt=onRoad>0;
 if(this.phase==='crashed'){const dec=Math.exp(-(r.dirt?2.2:.9)*dt);r.vx*=dec;r.vz*=dec;r.x+=r.vx*dt;r.z+=r.vz*dt;const b=this.board;b.vx*=Math.exp(-.35*dt);b.vz*=Math.exp(-.35*dt);b.x+=b.vx*dt;b.z+=b.vz*dt;if(this.phaseTime>3.8)this.phase='ended';return;}
 ...
 const lat=r.vx*nx+r.vz*nz,grip=b.grip*cfg.grip*novice*(crouch?1.15:1)*(r.dirt?.28:1)*(r.height>.05?.04:1);
 const lateralA=clamp(-lat*9,-grip,grip);
 ...
 const coastBrake=this.phase==='coasting'?clamp((this.phaseTime-2)*.25,0,.75):0;
 const resist=(r.dirt?1.35:.12)+.007*sp*sp+(slack&&c.brake?Math.max(0,sp-c.speed)*2:0)+coastBrake;
 ...
 if(r.height<.05){r.vx-=9.81*(height(r.x+.1,r.z)-height(r.x-.1,r.z))/.2*dt;r.vz-=9.81*(height(r.x,r.z+.1)-height(r.x,r.z-.1))/.2*dt;}
 r.tension=0;
 ...
 if(r.height>0||r.up>0){r.up-=9.81*dt;r.height=Math.max(0,r.height+r.up*dt);if(!r.height)r.up=0;}
 const slip=Math.abs(lat),load=Math.max(0,slip-.48);r.wobble=clamp(r.wobble+(load*.53+(r.dirt?.4:0)-(crouch?.65:.4))*dt,0,1.3);
 const nowRoad=roadDistance(r.x,r.z);
 if(this.prevRoad<=0&&nowRoad>0&&r.height<.16){const normalSpeed=(nowRoad-this.prevRoad)/dt;if(normalSpeed>1.25||sp>6.1)this.crash('CURB');else{r.vx*=.82;r.vz*=.82;r.wobble+=.35;}}
 this.prevRoad=nowRoad;
 ...
 if(Math.abs(along)<2.45&&Math.abs(across)<1&&this.time>2)this.crash('CAR');
 if(this.phase==='coasting'&&Math.hypot(r.vx,r.vz)<.3&&c.speed<.3)this.phase='regroup';
